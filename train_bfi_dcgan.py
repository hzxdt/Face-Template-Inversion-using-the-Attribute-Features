...
import os, shutil
from configs_MXnbnetDcgan import get_main_parser
...
# from facenet_tf.facenet_extract import FacenetExtractor
# facenet = FacenetExtractor('./pretrain_model/facenet_20170512-110547.pb', [0])
...
# extractor = featureExtractZoo('facenet', args, True, None)
...
#NOTE - train
def train(DG, INV, log_interval, device, data_loader, inv_optimizer, dg_optimizer, epoch, randFaceIter):
    ...
    for batch_idx in range(num_batch):
        response = requests.get('http://localhost:6666/predict')
        data = pickle.loads(response.content)

        imgs = torch.tensor((data['origin'] + 1) / 2).to(device)
        imgs = F.interpolate(imgs, size=(args.img_size, args.img_size))
...
def main():
    ...
    # img_gen = img_generator(ctx=devs, batch_size=args.batch_size,model_prefix=args.face_gen_prefix, load_epoch=0)
    # randFaceIter = RandFaceIter(batch_size=args.batch_size, img_gen=img_gen, fea_ext=fea_ext)
    # randFaceIter = mx.io.PrefetchingIter(randFaceIter)
    randFaceIter = None
    train_loader = None
    test1_loader = None
    ...
    isload = False
    if isload:
        path = f'/home/ljh/teststylegan/mystylegan/BFIRGB/out_dcgan_bfi5loss_facenet_nbalign/inversion_epoch7.pth'
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        time_interval = int(time.time())
        train(DG, INV, args.log_interval, device, train_loader, inv_optimizer, dg_optimizer, epoch, randFaceIter)
        print(f"\ttrain cost: {int(time.time())-time_interval}")

        if 'ffhq' in args.outdir or 'vggface' in args.outdir or 'dcgan' in args.outdir:
